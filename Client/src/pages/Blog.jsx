...
const Blog= ()=> {
  ...
  try {
    const { data } = await axios.post(`/api/blog/addcomments`, {
      blog: id,
      name,
      content,
    });
  ...
  return data ? (
    <div className='relative'>
      ...
      <div className='text-center mt-20'> 
        ...
        <h1 className='text-2xl sm:text-5xl font-semiboldmax-w-2xl mx-auto text-gray-800'>{data.title}</h1>
       <h2 className='my-5 max-w-lg truncate mx-auto'>{data.subTitle}</h2> 
       <p className='inline-block py-1 px-4 rounded-full mb-6 border text-sm bg-primary/5 font-medium text-primary'>Michael Brown</p>
      </div>
                ...
                <div className='flex flex-col gap-4 '>
                    {comments .map((item,index)=>(
                      ...
                      <div className="flex items-center">
                          ...
                          <p className="font-medium ml-4">{item.name}</p>
                      </div>

                      <p className='text0sm max-w-md mt-3 ml-30'>{item.content}</p>
